        ...
        viewModel: function (configuration) {
            ...
            this.itemsOnCurrentPage = ko.computed(function () {
                ...
                for (var i in this.filter) {
                    var search = this.filter[i]().toUpperCase();
                    if (search !== '') {
                        this.result(ko.utils.arrayFilter(this.data(), function (item) {
                            var value = (typeof item[i] === 'object') ? item[i]().toString() : item[i].toString();
                ...
                } else if (this.maxPageIndex() < this.currentPageIndex()) {
                    this.currentPageIndex(0);
                }

                var startIndex = this.pageSize * this.currentPageIndex();
                return ko.unwrap(this.result).slice(startIndex, startIndex + this.pageSize);
            }, this);

            this.maxPageIndex = ko.computed(function () {
                return Math.ceil(ko.unwrap(this.result).length / this.pageSize) - 1;
            }, this);
        }
    };
